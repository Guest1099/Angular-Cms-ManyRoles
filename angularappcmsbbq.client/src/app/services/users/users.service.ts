    ...
    this.http.get<any>(`${this.api}`).subscribe({
      next: ((result: TaskResult<ApplicationUser[]>) => {
        if (result.success) {
          ...
          let data = result.model as ApplicationUser[];
          this.users = data.sort((a, b) => a.email.localeCompare(b.email));
...
*/
  ...
  public create(form: FormGroup): void {
    ...
    this.http.post<any>(`${this.api}`, registerViewModel).subscribe({
  ...
  public edit(id: string, form: FormGroup): void {
    ...
    this.loadingElements = true;
    this.http.put<any>(`${this.api}/${id}`, user).subscribe({
      next: ((result: TaskResult<ApplicationUser>) => {
        if (result.success) {
