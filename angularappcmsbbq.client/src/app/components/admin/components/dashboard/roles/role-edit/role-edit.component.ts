    ...
    this.route.paramMap.subscribe(params => {
      ...
      if (id) {
        ...
        this.rolesService.get(id).subscribe({
          ...
          error: (error: Error) => {
            this.snackBarService.setSnackBar(`Brak połączenia z bazą danych or token time expired. ${InfoService.info('RoleEditComponent', 'get')}. Name: ${error.name}. Message: ${error.message}`);
          }
        });
