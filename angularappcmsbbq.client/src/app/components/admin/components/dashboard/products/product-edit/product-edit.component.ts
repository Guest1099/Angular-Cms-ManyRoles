...
export class ProductEditComponent implements OnInit {
    ...
    this.route.paramMap.subscribe(params => {
      ...
      if (id) {
        ...
        this.productsService.get(id).subscribe({
          ...
          error: (error: Error) => {
            this.snackBarService.setSnackBar(`Brak połączenia z bazą danych or token time expired. ${InfoService.info('ProductEditComponent', 'get')}. Name: ${error.name}. Message: ${error.message}`);
          }
        });
      }
    });
  ...
  getAllCategories(): void {
    this.categoriesService.getAll().subscribe({
      ...
      error: (error: Error) => {
        this.snackBarService.setSnackBar(`Brak połączenia z bazą danych or token time expired. ${InfoService.info('ProductEditComponent', 'getAllCategories')}. Name: ${error.name}. Message: ${error.message}`);
      }
    });
  }
  ...
  getAllSubcategories(categoryId: string): void {
    if (categoryId.length > 0) {
      this.subcategoriesService.getAllByCategoryId(categoryId).subscribe({
        ...
        error: (error: Error) => {
          this.snackBarService.setSnackBar(`Brak połączenia z bazą danych or token time expired. ${InfoService.info('ProductEditComponent', 'getAllSubcategories')}. Name: ${error.name}. Message: ${error.message}`);
        }
      });
    }
  }
  ...
  getAllSubsubcategories(categoryId: string, subcategoryId: string): void {
    if (categoryId.length > 0 && subcategoryId.length > 0) {
      this.subsubcategoriesService.getAllByCategoryIdAndSubcategoryId(categoryId, subcategoryId).subscribe({
        ...
        error: (error: Error) => {
          this.snackBarService.setSnackBar(`Brak połączenia z bazą danych or token time expired. ${InfoService.info('ProductEditComponent', 'getAllSubsubcategories')}. Name: ${error.name}. Message: ${error.message}`);
        }
      });
    }
  }
